...
| main_title | yes | Plain text | Preserve as the poster's main title and infer marketing intent. For `会员` A, use the entire title in bundled `SourceHanSerifSC-Heavy.otf`; an A `补充要求` containing `特殊字` uses the bundled brush-title reference. For `会员` S, pass the exact supplied copy to `--title`; it uses the 580px reference text region as a single line and proportionally shrinks when needed. Only a title containing X/x/× with non-empty text on both sides becomes the MasterGo split-title layout; both sides stay single-line at one shared fitted size. Otherwise no × is added. For `会员` B, use the modern display-Heiti treatment. |
...
| membership_theme | required for membership S | `圣诞` or `中秋` | Selects the membership S node background and theme focus. It does not choose the product relation. |
...
For `会员` A 2:1 member-day heads, render `main_title` as a separate transparent title PNG: use the bundled `SourceHanSerifSC-Heavy.otf` by default, and use the bundled brush-title reference only when `补充要求` explicitly contains `特殊字`. Treat `subtitle` as the date/standard-font copy composited after generation through the global subtitle typography policy. The fixed member-day mark, rule text, rule-button background, and bottom wave are compositor layers. `会员` S uses `membership-s-2x1`: after the selected node background passes visual QA, `membership_s_renderer.py` receives exact `--title` and `--subtitle` text inputs. They share the 580px reference text region; the main title stays single-line and proportionally shrinks to fit, while the subtitle can wrap and centers each line. The logo/title/subtitle group stays vertically centered. It tints the bundled `member-day-brand-mask.png` from the same background-adaptive color. `membership_theme` selects only 圣诞/中秋 visual routing and never supplies title content. `会员` B does not use this A composition route. For `会员` B 2:1, follow recycle B before rendering: generate a poster base without only the main title and subtitle, preserving all source-product text and screen content. Run `text_layout_renderer.py --profile membership-b-2x1 --trace-output <title-layout.json>`, then run `membership_head_renderer.py --mode membership-b --title-layout <title-layout.json>` to add `member-b-brand.png` above the local title.

For membership A/S 2:1 heads, accept at most four `product_assets`. Membership A uses the direct default foreground of gift boxes, ribbons, and coins with no assets, and its ordinary 1-4 product branch uses the fixed slots, size, angle, layering, partial-upload behavior, and automatic slot assignment in `assets/membership-head-template/template.json` under `product_slots`. Membership S uses its theme gift branch with no assets; with 1-4 assets, it first resolves `rel_single_hero`, `rel_primary_support`, or `rel_multi_collection`, then selects the matching shared product combination. The A lower foreground stays in the region from 70% canvas height to the bottom edge; only A uses the inside-box close-up and its 75%-width gift-box target. When more than four assets are supplied, do not generate and ask the user to keep at most four products.
