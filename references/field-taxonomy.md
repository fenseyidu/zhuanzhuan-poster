...
| membership composition | `references/membership-head-composition.md`, `references/prompt-assembly.md`, `assets/membership-head-template/` | A/S member-day 2:1 route: image-to-image base plus fixed code-drawn composition; B 2:1 uses `membership-b-2x1` to locally draw its title group, then lets the membership renderer add the alpha-tinted member mark from the local-title trace. |
