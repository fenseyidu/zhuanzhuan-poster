...
The lower foreground stays in the bottom 30% (`y=788` to `1125`). Only the ordinary-product slot foreground uses the inside-box close-up and the 75%-width gift-box opening and low-side-corner target; this is a soft initial-composition target, not a fixed dimension for local edits. The active local template owns title geometry: at 1125px reference width, A/S uses a 110px title size with 908px maximum visible width. A one-line title that exceeds 908px must wrap at a semantic boundary before composition. On the 2250px output canvas, the title box is `(217,360,1816,324)` and the composited title group has a fixed visible height of `220`; its visible ink must stay clear of the future member-day mark and date reservations.
