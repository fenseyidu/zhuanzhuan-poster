...
- For `会员` S, do not apply the A/S slot, title-asset, or fixed-layer checks below: those legacy A/S references now apply to A only. Check the selected 圣诞/中秋 background, the zero-product gift branch or resolved product relation, the left reading field, right theme focus, and shared theme light. For 圣诞 with zero product assets, verify a single main gift box fills the registered right-lower gift zone with clear top/front champagne-gold light, and verify that the left reading field keeps a visible, open pine-green midtone. For 圣诞 with supplied products, also compare the AI base before text composition with `source/membership-s-christmas-background-master.png`: retain the wide left reading field and the right star-crown/light-tree/gift structure, and confirm that the compact product group is enclosed by the lower inner tree light rather than pasted in front of it; its upper/front/rim light, warm ground reflection, fine particles, and soft contact shadow must read as one field. Then run `membership_s_renderer.py` and verify the complete alpha-mask logo, a single-line fitted main title, the conditional X-title behavior, and one background-adaptive color shared by logo and text.
...
- In the `会员` S `圣诞` `2:1` route with supplied products, the final Prompt explicitly keeps the group compact inside the lower inner volume of the light tree, specifies shared champagne-gold top/front/rim light, and requires a continuous gold ground reflection with soft contact shadow; it must not describe the tree as a backdrop for an independent product lineup.
...
- A `会员` S `圣诞` `2:1` Prompt with supplied products omits compact scale, light-tree containment, shared warm light, or the continuous ground-light transition, or permits a separately lit/cutout-like product group in front of the tree.
