...
Any FAIL in a required layer should trigger `需定向重生` when image generation is available. Default retry count is one; `会员` image generation is the exception and automatically allows up to two targeted regenerations, then stops without asking whether to continue.
...
- For `会员` B 2:1 layouts, use 1125×562 as the typography reference: main-title font size is 80px and maximum visible width is 590px. The subtitle follows the global subtitle typography policy, shares the main-title left edge by default, and is not reduced to tiny type. The left 60% remains a continuous text-reading background; the entire main product group stays inside the right-side 40% visual zone (`x=60%–100%`) with a safe outer margin, remains compact and staggered, and does not encroach on the title-reading area or fill the visual zone.
...
- For `biz_membership + cat_membership_day + S + 圣诞` with supplied products, the tree-base theme focus contains only the supplied product group, light-tree volume, particles, ground reflection, and contact shadow; no default gift box, packaging, ribbon, bow, or other physical gift object remains.
...
- For `biz_membership + cat_membership_day + S + 圣诞` with supplied products, the group is oversized enough to crowd the cone or make the light tree a backdrop; sits in front of rather than inside the tree’s lower light volume; has cold/independent studio lighting, hard cutout edges, isolated shadow, a ground transition disconnected from the gold particle field, or any default gift box, packaging, ribbon, bow, or other physical gift object remains. Mark this as `商品层 + 背景层` and use the dedicated light-integration correction.
