...
`biz_membership + cat_membership_day` has a fixed member-day route. B inherits the recycle B visual route only. A is a title-free membership 2:1 AI base plus code-composition route; S inherits A. The saved MasterGo `source/member-day-background-master.png` is the base reference: retain the warm-gold background, lower foreground group, coins, small toy, overlap, perspective, and lighting, but keep the title/fixed-layer area clean. A separate title-only image task treats `member-day-title-style-reference.png` as a reference-image text replacement: preserve its brush treatment and replace only its visible title copy with the requested main title; do not add independent style descriptors that reinterpret the brushwork. It must return a transparent title PNG. The composer places it in `template.json.title_layer.box`, then adds the mark, date, adaptive rule badge, and bottom wave. On the base generation, constrain the product-and-coin main visual to the bottom 30% of the canvas with no product subject visibly higher than 72%. Product-slot assignment, partial bottom emergence, and the four-product maximum remain unchanged.
...
Before A/S composition, independently check the title-free base and title PNG. A product-region, slot, crop, or overlap failure retries only the current AI base; a title-copy, brush-style, transparency, title-box, or mark/date-reservation failure retries only the title asset. Only the final composed image checks all layers together for collisions. This route does not apply to membership B.
