...
- 对 `会员` A/S 2:1，标题与下方商品组是不可拆分的联动主视觉。任何标题框、标题预留区碰撞、商品区上侵、商品槽位几何或前后遮挡失败，都必须先归为一次“主视觉联动版式失败”，以标题框、日期留白和下方商品区为共同锚点进行一条整体校正；联动预检通过前不得贴固定层。
