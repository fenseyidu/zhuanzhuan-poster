...
- For `会员` A/S `2:1`, `template.json.title_layer.layout` is the title source of truth: at a 1125px-wide reference canvas, use 110px title size and 908px maximum visible width. If one line exceeds that width, wrap at a semantic boundary before composition; do not decide from product-area pressure or subjective readability. For `会员` B `2:1`, use `assets/text-layouts/membership/b-2x1.json`: 80px title size and 590px maximum width at the same reference width.
...
- QA stays lightweight: do not create standalone QA reports unless the user asks. PASS continues to the next step; FAIL defaults to one targeted retry; if the retry still fails, state the blocker. For `会员` image generation, override this failure handling: when the output does not meet the applicable visual standard, do not automatically retry. Ask the user whether to continue targeted regeneration. Only after the user replies `继续`, use the current failed generation as the sole edit target for one targeted regeneration, then QA again. If it still fails, ask the same question again; repeat until the result passes or the user chooses to stop. Each retry must change only the currently failed item and preserve previously passed content.
